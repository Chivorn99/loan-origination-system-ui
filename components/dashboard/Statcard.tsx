...
export function StatCard({ variant = 'DEFAULT', label, value, change, subtitle, icon, isLoading }: StatCardProps) {
  if (isLoading) {
    return (
      <div className="flex animate-pulse flex-col gap-3 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
  ...
  return (
    <div className={`flex flex-col gap-3 rounded-2xl border bg-white p-5 shadow-sm ${cardStyles[variant]}`}>
      ...
      <div className="flex items-center justify-between">
        ...
        <span className={`flex h-9 w-9 items-center justify-center rounded-xl ${iconStyles[variant]}`}>{icon}</span>
      </div>

      <div className="flex items-end gap-2">
        <span className="text-3xl font-bold tracking-tight text-gray-900">{value}</span>
        {change && <span className="mb-0.5 text-sm font-semibold text-red-500">{change}</span>}
      </div>
